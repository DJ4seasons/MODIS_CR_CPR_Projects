...
This code is for cross-validation to decide an optimal alpha
...
def main(model,rg_nm,tgt_crs):
    ...
    groups = create_year_groups(
        n_years=n_tr_years, n_folds=n_folds, n_points_per_year=npt,  
        random_state=1234
    )
...
def plot(model, cv_data, rg_names, tgt_crs, out_fn):
    ...
    suptit= f'{model}: RMSE by Ridge_alpha'
    ...
    for k,data0 in enumerate(cv_data):
        ...
        for j,data1 in enumerate(data0):
            pic1= ax1.semilogx(data1[:,0],data1[:,1],
                           lw=1.5,ls=ls[j%len(ls)],c=cc[j//(n_rg//len(cc))],
                           label=rg_names[j])
            y_min_idx= np.argmin(data1[:,1])
            sct1= ax1.scatter([data1[y_min_idx,0],],[data1[y_min_idx,1],],s=20,c=cc[j//(n_rg//len(cc))],marker='v')
            #y_max_idx= np.argmax(data1[:,1])
            #sct2= ax1.scatter([data1[y_max_idx,0],],[data1[y_max_idx,1],],s=12,c=cc[j//(n_rg//len(cc))],marker='^')
        ...
        ix+= lx+gapx
        if ix+gapx>rf:
            ax1.legend(loc='upper left',fontsize=10,framealpha=0.9, borderaxespad=0.,bbox_to_anchor=(1.03,1.))
    ...
    print(model,'Best_alpha=',data1[y_min_idx,0])
...
if __name__=="__main__":
    ...
    cv_score_all=[]
    for rg_idx in range(6):
        cvs= main(model, rg_names[rg_idx],tgt_crs)
        cv_score_all.append(cvs)

    cv_score_all= np.asarray(cv_score_all).swapaxes(0,1)  #[ncr,n_rg,n_alp,2]
    out_fig= f'./Pics/FigX01.Ridge_alpha_CV.{model}.png'
    plot(model, cv_score_all,rg_names,tgt_crs,out_fig)
